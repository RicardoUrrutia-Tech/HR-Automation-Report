        ...
        if not mes or not anio:
            ...
            with pd.ExcelWriter(output, engine="xlsxwriter", datetime_format='dd/mm/yyyy', date_format='dd/mm/yyyy') as writer:
                ...
                for i, nombre in enumerate(nombres, start=2):
                    ...
                    fila = i + 1
                    ws_inc.write_formula(f"B{fila}", f"=SUMIF(Detalle!$C:$C,A{fila},Detalle!{col_retraso}:{col_retraso})")
                    ws_inc.write_formula(f"C{fila}", f"=SUMIF(Detalle!$C:$C,A{fila},Detalle!{col_salida}:{col_salida})")
